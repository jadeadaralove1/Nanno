...
export default {
  ...
  run: async (client, m, args, command) => {
    ...
    if (!text) {
      return m.reply('𐄹 ۪ ׁ 💉ᩚ̼ 𖹭̫ ▎ Por favor, ingresa un término de búsqueda o un enlace de Pinterest.')
    }
    try {
      if (isPinterestUrl) {
...
if (!results || results.length === 0) {
  return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ No se encontraron resultados para *${text}*.`)
}
