...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (user.lastApuesta && ahora - user.lastApuesta < tiempoEspera) {
      ...
      return client.reply(m.chat, `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ Debes esperar *${tiempoRestante}* para usar *${usedPrefix + command}* nuevamente.`, m)
    }
    ...
    if (args.length < 1) {
      return client.reply(m.chat, `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎Ingresa la cantidad de *${currency}* que deseas aportar contra *${botname}*\n> Ejemplo: *${usedPrefix + command} 100*`, m)
    }
    if (user.coins >= count) {
      user.coins -= count
      let resultado = ''
      ...
      client.reply(m.chat, `𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ No tienes *¥${formatNumber(count)} ${currency}* para apostar!`, m)
    }
  }
}
