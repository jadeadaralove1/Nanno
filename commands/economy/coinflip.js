export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (!isNaN(a0)) {
      ...
      return m.reply(`𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ Cantidad inválida, ingresa un número válido.\n> Ejemplo » *${usedPrefix + command} 200 cara* o *${usedPrefix + command} cruz 200*`)
    }
    if (Math.abs(cantidad) < 100) {
      return m.reply(`𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ La cantidad mínima para apostar es *100 ${monedas}*.`)
    }
    if (!['cara', 'cruz'].includes(eleccion)) {
      return m.reply(`𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ Elección inválida. Solo se admite *cara* o *cruz*.\n> Ejemplo » *${usedPrefix + command} 200 cara*`)
    }
    if (cantidad > user.coins) {
      return m.reply(`𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ No tienes suficientes *${monedas}* fuera del banco para apostar, tienes *¥${user.coins.toLocaleString()} ${monedas}*.`)
    }
    ...
    const mensaje = `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎La moneda ha caído en *${capitalize(resultado)}* y has ${acierto ? 'ganado' : 'perdido'} *¥${Math.abs(cambio).toLocaleString()} ${monedas}*!\n> Tu elección fue *${capitalize(eleccion)}*`
