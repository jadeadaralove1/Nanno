export default {
  ...
  run: async (client, m, args, usedPrefix) => {
    ...
    if (user.health < 5) return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ No tienes suficiente salud para volver a *cazar*.\n> Usa *"${usedPrefix}heal"* para curarte.`)
    const remaining = user.lastmine - Date.now()
    if (remaining > 0) {
      return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Debes esperar *${msToTime(remaining)}* para minar de nuevo.`)
    }
    ...
    if (isLegendary) {
      ...
      bonusMsg = '\n𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ Recompensa ÉPICA obtenida!'
    } else {
      ...
      if (Math.random() < 0.1) {
        ...
        reward += bonus
        bonusMsg = `\n𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ ¡Bonus de minería! Ganaste *${bonus.toLocaleString()}* ${monedas} extra`
      }
    }
    user.coins += reward
    ...
    let msg = `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ ${narration} *${reward.toLocaleString()} ${monedas}*`
    if (bonusMsg) msg += `\n${bonusMsg}`
