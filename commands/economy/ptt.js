export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (remainingTime > 0)
      return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Debes esperar *${msToTime(remainingTime)}* antes de jugar nuevamente.`)
    const options = ['piedra', 'papel', 'tijera']
    const userChoice = args[0]?.trim().toLowerCase()
    if (!options.includes(userChoice)) return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Usa el comando así:\n› *${usedPrefix}ppt piedra*, *papel* o *tijera*`)
    ...
    if (result === 'win') {
      ...
      await client.sendMessage(chatId, { text: `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ Perdiste.\n\n> *Tu elección ›* ${userChoice}\n> *${botname} eligió ›* ${botChoice}\n> *${monedas} ›* -¥${actualLoss.toLocaleString()}` }, { quoted: m })
    } else {
      user.coins += tieReward
      await client.sendMessage(chatId, { text: `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎Empate.\n\n> ♡ *Tu elección ›* ${userChoice}\n> *${botname} eligió ›* ${botChoice}\n> *${monedas} ›* +¥${tieReward.toLocaleString()}` }, { quoted: m })
    }
