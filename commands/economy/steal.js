...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (Date.now() < user.laststeal) {
      ...
      return client.reply(m.chat, `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎Debes esperar *${formatTime(restante)}* para usar *${usedPrefix + command}* de nuevo.`, m)
    }
    const mentioned = m.mentionedJid || []
    const who2 = mentioned[0] || (m.quoted ? m.quoted.sender : null)
    const who = await resolveLidToRealJid(who2, client, m.chat)
    if (!who) return client.reply(m.chat, `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ Debes mencionar a alguien para intentar robarle.`, m)
    if (!(who in db.chats[m.chat].users)) {
      return client.reply(m.chat, `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ El usuario no se encuentra en mi base de datos.`, m)
    }
    ...
    if (chance < 0.3) {
      ...
      return client.reply(m.chat, `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ El robo salió mal y perdiste *¥${loss.toLocaleString()} ${currency}*.`, m)
    }
    const rob = Math.floor(Math.random() * (8000 - 4000 + 1)) + 4000
    if (target.coins < rob) {
      return client.reply(m.chat, `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ *${name}* no tiene suficientes *${currency}* fuera del banco como para que valga la pena intentar robar.`, m, { mentions: [who] })
    }
    user.coins += rob
    target.coins -= rob
    user.laststeal = Date.now() + 3600000
    client.reply(m.chat, `𐄹 ۪ ׁ 🪙ᩚ̼ 𖹭̫ ▎ Le robaste *¥${rob.toLocaleString()} ${currency}* a *${name}*`, m, { mentions: [who] })
  }
}
