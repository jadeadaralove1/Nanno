...
> ⠾ 🥿  Hi, Soy nanno la Chica Nueva. Creada en codigos y comandos.  Para saber mas de mi usa #menu . 

> ⠾ 🪦  Quiero darte la bienvenida. 
 
                          ૮₍｡´• ˕ •'｡₎ა  

   ㅤ    𝗥𝖾𝖼𝗎𝖾𝗋𝖽𝖺  𝗅𝖾𝖾𝗋  𝗅𝖺𝗌  𝗋𝖾𝗀𝗅𝖺𝗌
...
╰─────────────────╯`
        ...
        if (anu.action === 'promote' && chat?.alerts && (!primaryBotId || primaryBotId === botId)) {
          ...
          await client.sendMessage(anu.id, { text: `𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ *@${phone}* ha sido promovido a Administrador por *@${usuario.split('@')[0]}.*`, mentions: [jid, usuario, ...groupAdmins.map(v => v.id)] })
        }
        if (anu.action === 'demote' && chat?.alerts && (!primaryBotId || primaryBotId === botId)) {
          const usuario = anu.author
          await client.sendMessage(anu.id, { text: `𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ *@${phone}* ha sido degradado de Administrador por *@${usuario.split('@')[0]}.*`, mentions: [jid, usuario, ...groupAdmins.map(v => v.id)] })
        }
      }
    } catch (err) {
  ...
  if (m.messageStubType == 21) {
    await client.sendMessage(id, { text: `𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ @${phone} cambió el nombre del grupo a *${m.messageStubParameters[0]}*`, mentions: [actor, ...groupAdmins.map(v => v.id)] })
  }
  if (m.messageStubType == 22) {
    await client.sendMessage(id, { text: `𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ @${phone} cambió el icono del grupo.`, mentions: [actor, ...groupAdmins.map(v => v.id)] })
  }
  if (m.messageStubType == 23) {
    await client.sendMessage(id, { text: `𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ @${phone} restableció el enlace del grupo.`, mentions: [actor, ...groupAdmins.map(v => v.id)] })
  }
  if (m.messageStubType == 24) {
    await client.sendMessage(id, { text: `𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ @${phone} cambió la descripción del grupo.`, mentions: [actor, ...groupAdmins.map(v => v.id)] })
  }
  if (m.messageStubType == 25) {
    await client.sendMessage(id, { text: `𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ @${phone} cambió los ajustes del grupo para permitir que ${m.messageStubParameters[0] == 'on' ? 'solo admins' : 'todos'} puedan configurar el grupo.`, mentions: [actor, ...groupAdmins.map(v => v.id)] })
  }
  if (m.messageStubType == 26) {
    await client.sendMessage(id, { text: `𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎@${phone} cambió los ajustes del grupo para permitir que ${m.messageStubParameters[0] === 'on' ? 'solo los administradores puedan enviar mensajes al grupo.' : 'todos los miembros puedan enviar mensajes al grupo.'}`, mentions: [actor, ...groupAdmins.map(v => v.id)] })
  }
})
}