  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
     ...
     const who = await resolveLidToRealJid(who2, client, m.chat);
      if (!who2) {
        return client.reply(m.chat, `𐄹 ۪ ׁ 💉ᩚ̼ 𖹭̫ ▎Por favor menciona un bot para convertirlo en primario.`, m)
      }
      ...
      if (!allowedBots.includes(who)) {
        return client.reply(m.chat, `𐄹 ۪ ׁ 💉ᩚ̼ 𖹭̫ ▎El usuario mencionado no es una instancia de Sub-Bot.`, m)
      }
      if (!groupParticipants.includes(who)) {
        return client.reply(m.chat, `𐄹 ۪ ׁ 💉ᩚ̼ 𖹭̫ ▎ El bot mencionado no está presente en este grupo.`, m)
      }
      if (chat.primaryBot === who) {
        return client.reply(m.chat, `𐄹 ۪ ׁ 🥌ᩚ̼ 𖹭̫ ▎ @${who.split('@')[0]} ya es el Bot principal del Grupo.`, m, { mentions: [who] })
      }
      chat.primaryBot = who
      await client.reply(m.chat, `𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎Se ha establecido a @${who.split('@')[0]} como bot primario de este grupo.\n
