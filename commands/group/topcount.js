...
export default {
  ...
  run: async (client, m, args, command, text, prefix) => {
    ...
    if (daysArg < 1) return m.reply(`𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ El número de días debe ser mayor a 0.`)
    ...
    if (ranking.length === 0) return m.reply(`𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ No hay actividad registrada en los últimos ${daysArg} días.`)
    const page = parseInt(args[1]) || 1
    const perPage = 10
    const totalPages = Math.ceil(ranking.length / perPage)
    if (page < 1 || page > totalPages) return m.reply(`𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ Página inválida. Solo hay ${totalPages} páginas disponibles.`)
    ...
    let report = `𐄹 ۪ ׁ 🥀ᩚ̼ 𖹭̫ ▎Top de mensajes en los últimos *${daysArg}* día${daysArg > 1 ? 's' : ''}\n\n`
