  ...
  run: async (client, m, args) => {
   ...
   const userId = await resolveLidToRealJid(who2, client, m.chat);
    if (!who2 || !chat.users[userId]) {
      return m.reply('𐄹 ۪ ׁ 🥀ᩚ̼ 𖹭̫ ▎ Menciona o responde a un usuario válido para ver sus advertencias.')
    }
    const user = chat.users[userId]
    const total = user.warnings?.length || 0
    if (total === 0) {
      return client.reply(m.chat, `𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ @${userId.split('@')[0]} no tiene advertencias registradas.`, m, { mentions: [userId] })
    }
