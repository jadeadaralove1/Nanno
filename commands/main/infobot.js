...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    try {
    const message = `𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎*Información del bot* ◢ ${botname} ◤
...
│ Moneda       › *${monedas}*
╰─────────────
𐄹 ۪ ׁ 😺ᩚ̼ 𖹭̫ ▎ *Prefijo${Array.isArray(prefijo) && prefijo.length > 1 ? 's' : ''} ›* ${prefijo === true ? '`sin prefijos`' : (Array.isArray(prefijo) ? prefijo : [prefijo || '/']).map(p => `\`${p}\``).join(', ')}
...
╰─────────────
𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ *${desar === 'Hombre' ? 'Dueño' : desar === 'Mujer' ? 'Dueña' : 'Dueño(a)'} ›* ${owner ? (!isNaN(owner.replace(/@s\.whatsapp\.net$/, '')) ? `@${owner.split('@')[0]}` : owner) : "Oculto por privacidad"}
...
await client.sendMessage(m.chat, banner.includes('.mp4') || banner.includes('.webm') ? {
            ...
            contextInfo: {
              ...
              externalAdReply: {
                title: botname,
                body: `${namebot}, made love by jade`,
