  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (command === 'addcoin') {
        ...
        if (isNaN(coinTxt)) return client.reply(m.chat, '𐄹 ۪ ׁ ⚠️ᩚ̼ 𖹭̫ ▎ Solo se permiten números.', m)       
        await m.react('🕒')
        ...
        await m.react('✔️')
        return client.reply(m.chat, `𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ *Añadido:*\n» ${dmt} ${currency}\n@${who.split('@')[0]}, recibiste ${dmt} ${currency}`, m, { mentions: [who] })
      }
      if (command === 'addxp') {
        if (!who) return client.reply(m.chat, '𐄹 ۪ ׁ ⚠️ᩚ̼ 𖹭̫ ▎ Por favor, menciona al usuario o cita un mensaje.', m)
        const xpTxt = args.find(arg => !isNaN(arg) && !arg.includes('@'))
        if (!xpTxt) return client.reply(m.chat, '𐄹 ۪ ׁ ⚠️ᩚ̼ 𖹭̫ ▎ Ingresa la cantidad de experiencia (XP) que deseas añadir.\nEjemplo: !addxp @usuario 50', m)
        if (isNaN(xpTxt)) return client.reply(m.chat, '𐄹 ۪ ׁ ⚠️ᩚ̼ 𖹭̫ ▎ Solo números son permitidos.', m)
        await m.react('🕒')
        const xp = parseInt(xpTxt)
        if (xp < 1) {
          await m.react('✖️')
          return client.reply(m.chat, '𐄹 ۪ ׁ ⚠️ᩚ̼ 𖹭̫ ▎ El mínimo de experiencia (XP) es *1*', m)
        }
        ...
        userData[who].exp += xp
        await m.react('✔️')
        return client.reply(m.chat, `𐄹 ۪ ׁ ⚠️ᩚ̼ 𖹭̫ ▎ XP Añadido: *${xp}*\n@${who.split('@')[0]}, recibiste ${xp} XP`, m, { mentions: [who] })
      }
    } catch (error) {
      console.error(error)
      await m.react('✖️')
      return client.reply(m.chat, `𐄹 ۪ ׁ ⚠️ᩚ̼ 𖹭̫ ▎ Se ha producido un problema.\n${error.message}`, m)
    }
  }
}