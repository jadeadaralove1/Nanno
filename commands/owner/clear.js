export default {
  ...
  run: async (client, m, args) => {
    ...
    if (!chat?.users || typeof chat.users !== 'object')
      return m.reply('૮₍ ៸៸ ᵜ ก ₎ა No se encontraron datos del grupo.')
    ...
    if (!eliminados)
      return m.reply('𐄹 ۪  No se encontraron usuarios inactivos.\n> ⴵ Tiempo limite: 40 dias')
    const report = ['૮₍ ៸៸ ᵜ ก ₎ა Se han eliminado los datos de los usuarios inactivos.', `> ♡ Claims eliminados: ${waifus}`, `> ⛁ Coins eliminados: ${dinero.toLocaleString()}`, `> ❖ Usuarios inactivos: ${eliminados}`, `> ⴵ Tiempo límite: 40 días`, `> ❏ El comando se ejecutó en ${getDuration(start)}ms`, '', ...userList,].join('\n')
