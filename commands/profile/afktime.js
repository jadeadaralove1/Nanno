...
await client.reply(m.chat, `𐄹 ۪ ׁ 🥀ᩚ̼ 𖹭̫ ▎ *${global.db.data.users[m.sender].name || 'Usuario'}* Dejaste de estar inactivo.\n> ♡ Motivo » *${user.afkReason || 'sin especificar'}*\n> ♡ Tiempo inactivo » *${tiempo}* ${recompensa}`, m)
user.afk = -1
user.afkReason = ''
}
...
const tiempo = formatTiempo(ms)
await client.reply(m.chat,`𐄹 ۪ ׁ 🥀ᩚ̼ 𖹭̫ ▎ El usuario *${global.db.data.users[jid].name || 'Usuario'}* está AFK.\n> ♡ Motivo » *${target.afkReason || 'sin especificar'}*\n> ♡ Tiempo inactivo » *${tiempo}*`, m)
}}