...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (new Date() - global.db.data.users[m.sender].Subs < 120000) {
      return client.reply(m.chat,`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Debes esperar *${msToTime(time - new Date())}* para volver a intentar vincular un socket.`, m)
    }
    ...
    if (subsCount >= maxSubs) {
      return client.reply(m.chat, '𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ No se han encontrado espacios disponibles para registrar un `Sub-Bot`.', m)
    }
    commandFlags[m.sender] = true
    const rtx = '`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎` Vincula tu *cuenta* usando el *codigo.*\n\n> ♡ Sigue las *instrucciones*\n\n*›* Click en los *3 puntos*\n*›* Toque *dispositivos vinculados*\n*›* Vincular *nuevo dispositivo*\n*›* Selecciona *Vincular con el número de teléfono*\n\nꕤ *`Importante`*\n> 𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Este *Código* solo funciona en el *número que lo solicito*'
    const rtx2 = "`♡` Vincula tu *cuenta* usando *codigo qr.*\n\n> ♡ Sigue las *instrucciones*\n\n*›* Click en los *3 puntos*\n*›* Toque *dispositivos vinculados*\n*›* Vincular *nuevo dispositivo*\n*›* Escanea el código *QR.*\n\n> 𐄹 ۪ ׁ ⚠️ᩚ̼ 𖹭̫ ▎ Recuerda que no es recomendable usar tu cuenta principal para registrar un socket."
