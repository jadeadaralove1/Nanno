...
export default {
  ...
  run: async (client, m, args, usedPrefix, command, text) => {
    try {
      ...
      if (!text) return client.reply(m.chat, '𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Por favor, responde a un mensaje o ingresa un texto para crear el Sticker.', m)
      await m.react('🕒')
      let user = globalThis.db.data.users[m.sender] || {}
      const name = user.name || m.sender.split('@')[0]
      let texto1 = user.metadatos || `Nanno botꜱ`
