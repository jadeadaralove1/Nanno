...
export default {
  ...
  run: async (client, m, args, usedPrefix, command, text) => {
    try {
      if (!args[0]) return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Ingresa 2 emojis para combinar.\n> Ejemplo: *${usedPrefix + command}* 👻+👀`)
      let [emoji1, emoji2] = text.split`+`
      await m.react('🕒')
      let user = globalThis.db.data.users[m.sender] || {}
      const name = user.name || m.sender.split('@')[0]
      let texto1 = user.metadatos || `Nanno bots`
      ...
      if (!res.results || res.results.length === 0) throw new Error('𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ No se encontraron stickers para esos emojis.')
