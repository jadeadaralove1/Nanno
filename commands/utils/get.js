...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    const text = args[0];
    if (!text) return m.reply('𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Ingresa un enlace para realizar la solicitud.')
    if (!/^https?:\/\//.test(text)) {
      return m.reply('𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Ingresa un enlace válido que comience con http o https');
    }
    try {
      ...
      if (contentLength > 100 * 1024 * 1024) {
        return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ El archivo es demasiado grande.\nContent-Length: ${contentLength} bytes`);
      }
