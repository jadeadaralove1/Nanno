...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (!mime) return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Responde a una *imagen* con:\n${usedPrefix + command}`)
      if (!/^image\/(jpe?g|png|webp)$/i.test(mime)) return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ El formato *${mime || 'desconocido'}* no es compatible`)

      const buffer = await q.download?.()
      if (!buffer || !Buffer.isBuffer(buffer) || buffer.length < 10) return m.reply('𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ No se pudo descargar la imagen')

      const ft = await safeFileType(buffer)
      const inputMime = ft?.mime || mime || 'image/jpeg'
      if (!/^image\/(jpe?g|png|webp)$/i.test(inputMime)) return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ El formato *${inputMime}* no es compatible`)
      ...
      if (!result?.ok || !result?.buffer) {
        ...
        return m.reply(`𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ No se pudo *mejorar* la imagen (${msg})`)
      }
