...
export default {
  ...
  run: async (client, m, args, usedPrefix, command, text) => {
    ...
    try {
      ...
      if (inviteUrl) {
        try {
          ...
          m.reply('𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Grupo no encontrado.')
          return
        }
      }
    }
    ...
    const mentions = (res?.participants || inviteInfo?.participants || []).filter(p => p && p.id && (p.admin === "admin" || p.admin === "superadmin" || p.id === (res?.owner || inviteInfo?.owner))).map(p => p.id).filter(id => id && typeof id === 'string' && id.includes('@'))
      ...
      let newsletterInfo
      if (!channelUrl) return client.reply(m.chat, "𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Verifique que sea un enlace de canal de WhatsApp.", m)
      if (channelUrl) {
        try {
          newsletterInfo = await client.newsletterMetadata("invite", channelUrl).catch(() => null)
          if (!newsletterInfo) return client.reply(m.chat, "𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ No se encontró información del canal. Verifique que el enlace sea correcto.", m)
