...
export default {
  ...
  run: async (client, m, args, usedPrefix, command, text) => {
    try {
      ...
      if (!textFinal) return client.reply(m.chat, `𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ Ingresa un texto para crear el sticker.`, m)
      ...
      if (textFinal.length > 30) {
        await m.react('✖️')
        return client.reply(m.chat, `𐄹 ۪ ׁ 🤖ᩚ̼ 𖹭̫ ▎ El texto no puede tener más de 30 caracteres.`, m)
      }
      await m.react('🕒')
      const quoteObj = {
        type: 'quote',
      ...
      let texto1 = user.metadatos || `Nanno-bot`
