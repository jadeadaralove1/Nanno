...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      let texto1 = user.metadatos || `Nannobot`;
      ...
      if (/image/.test(mime) || /webp/.test(mime)) {
        ...
        if ((quoted.msg || quoted).seconds > 20) {
          return m.reply('ूᰍ ֔ ⠾ 💉 El video no puede ser muy largo');
        }        
        let buffer = await quoted.download();
        ...
        if (!url.match(/\.(jpe?g|png|gif|webp|mp4|mov|avi|mkv|webm)(\?.*)?$/i)) {
          return client.reply(m.chat, 'ूᰍ ֔ ⠾ 💉 La URL debe ser de una imagen (jpg, png, gif, webp) o video (mp4, mov, avi, mkv, webm)', m);
        }        
        const response = await fetch(url);
        if (!response.ok) return client.reply(m.chat, 'ूᰍ ֔ ⠾ 💉 No pude descargar ese archivo desde la URL.', m);
        ...
        return client.reply(m.chat, `ूᰍ ֔ ⠾ 💉 Por favor, envía una imagen, video, sticker o URL para hacer un sticker.\n> Usa *${usedPrefix + command} -list* para ver formas y efectos`, m);
      }
    } catch (e) {
